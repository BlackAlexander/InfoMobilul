    ...
    public partial class MainForm : Form
    {
        ...
        private void IncarcaUtilizator()
        {
            ...
            cmd.CommandText = String.Format(
                "SELECT Prenume, Id FROM Utilizatori WHERE Id IN (SELECT IdUtilizator FROM Logari WHERE DataLogare = (SELECT MAX(DataLogare) FROM Logari))"
                );
        ...
        private void IncarcaGraficInregistrari()
        {
            ...
            string[] numeLuni = { "", "Jan-19", "Feb-19", "Mar-19", "Apr-19", "May-19", "Jun-19", "Jul-19", "Aug-19", "Sep-19", "Oct-19", "Nov-19", "Dec-19" };

            for (int i = 1; i <= 12; i++) chart3.Series[0].Points.AddXY(numeLuni[i], luni[i]);
